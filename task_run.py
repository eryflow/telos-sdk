...
def main(argv: list[str] | None = None) -> int:
    ...
    with SQLiteTraceStore(home / "telos.db") as store:
        ...
        if args.command == "finish":
            ...
            print(f"TaskRun {run['id']} → {run['status']}")
            return 0
        task = store.ensure_task_type(args.task)
        state = store.get_evolution(task["id"])
        if state["task_type"]["production_profile_revision_id"] is None:
            create_profile(
                store, task_type=args.task, state="production", home=home,
                instructions=(
